...
Вторая строка содержит заголовки полей, позволяющие находить нужные столбцы в таблице. Например, если во втором столбце содержится строка "id", значит параметр id находится в строках данных во втором столбце. Таким образом, при изменении формата файла и добавлении столбцов в таблицу не нужно будет переписывать алгоритм парсинга, и можно унифицировать чтение всех csv файлов. Программе парсинга будет достаточно лишь знать, какие поля надо прочитать из файла.
Третья и все остальные строки содержат данные. При этом, некоторые из строк могут быть пустыми (ни один столбец не содержит данных), они должны быть проигнорировны.
