...
- `automatic_generators[key]` - словарь ООП-объектов автоматических генераторов по ключу "x_y"
...
### **6. Debug Logger (Система отладки)**
...
- tokens[key] через create_token_on_board() (централизованное создание всех фишек)
...
4. **Модульность** - используйте отдельные модули для переиспользуемых компонентов
5. **Конфигурируемость** - выносите данные в CSV файлы
6. **ООП для сложной логики** - используйте объекты для инкапсуляции состояния и поведения
7. **Безопасность типов** - всегда проверяйте типы перед вызовом методов
8. **Контролируемое логирование** - используйте `debug_logger` для отладочных сообщений
