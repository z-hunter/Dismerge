...
### **Обзор:**
Генераторы — это специальные фишки, которые могут создавать новые фишки после активации. Генераторы имеют ограниченную емкость и требуют перезарядки после использования.

### **Типы генераторов:**
- **Ручная активация** — активируются двойным кликом
- **Автоматическая активация** — активируются автоматически (в разработке)

### **Визуальные индикаторы:**
1. **Иконка готовности** — анимированная иконка энергии, показывающая что генератор готов к активации
2. **Индикатор перезарядки** — круговой индикатор прогресса во время перезарядки
...
```csv
Generator_ID,Type,Capacity,Reload_sec,Output_evo_id,Output_level,Weight
GEN_P,manual,3,5.0,P_PIN,1,100
GEN_F,manual,2,3.0,FNS,1,80
```

**Поля:**
- `Generator_ID` — идентификатор генератора (формат: "evo_id-level")
- `Type` — тип активации ("manual" или "auto")
- `Capacity` — количество активаций до перезарядки
- `Reload_sec` — время перезарядки в секундах
...
- Модульный круговой индикатор прогресса
- Система генераторов с ручной активацией
- Анимированные иконки готовности генераторов
- Индикаторы перезарядки генераторов
- Полная техническая документация

### **🔄 В разработке:**
- Система генераторов с автоматической активацией
- Расширенные типы генераторов
...
3. **Модульность** - используйте отдельные модули для переиспользуемых компонентов
4. **Конфигурируемость** - выносите данные в CSV файлы
