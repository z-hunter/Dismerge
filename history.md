# История версий Dismerge (current version = 0.0.4)
...
- **Конфигурация генераторов**: Модуль `scripts/generator_config.lua` для загрузки конфигурации генераторов из `config/gen.csv`
- **Интеграция генераторов**: Автоматическая загрузка и валидация конфигурации генераторов при старте игры
...
- **Устаревшие функции**: Заменен `table.getn()` на современный способ подсчета элементов
- **Ошибка отладочного вывода**: Исправлена проблема с nil значениями в `generator_config.debug_print_config()`
- **Неправильное количество столбцов**: Исправлен парсинг `config/init.csv` - убрано поле "8" (игровое поле имеет только 7 столбцов)
- **Ошибка строковых свойств при мерже**: Исправлена передача `evo_id` через `factory.create` в `perform_merge` - теперь используется сообщение `set_evo_id`
...
### 📋 API модуля generator_config.lua
```lua
-- Загрузка и валидация
local generators = generator_config.load_generator_config()
local is_valid = generator_config.validate_generator_config(evolution_tables)

-- Работа с генераторами
local gen = generator_config.get_generator("GEN_P-6")
local is_gen = generator_config.is_generator(token_id)
...
### 📋 Структура config/gen.csv
```csv
Generators,,,,,,,,,,,,,
,id,comment,Dispose after,Dispose to,M:Capacity,M:Reload(sec),M:Output,M:Rate,A:Capacity,A:Timer(sec),A:Reload(sec),A:Output,A:Rate
,GEN_P-6,компост,,,20,10,P_APP-1,5,,,,,
,,,,,,,P_APP-2,1,,,,,
...
```
...
9. **Готовность к генераторам**: Система готова для реализации механики генераторов
10. **Многострочные записи**: Поддержка сложных конфигураций с группировкой данных
