...
- В отличии от ручных генераторов, плявдяющихся на поле готовыми к генерации, автоматические появляются на поле в состоянии перезарядки, то есть после появления они сначала перезаряжаются, а потом генерируют фишки с заданным интервалом ( A:Timer(sec) ) до следующей перезарядки;
...
- Поскольку в одной фишке могут быть одновременно и ручной и автоматический генераторы, который совершенно независимы друг от друга, то возможны ситуации, когда на ней будут отображаться сразу оба индикатора. 
